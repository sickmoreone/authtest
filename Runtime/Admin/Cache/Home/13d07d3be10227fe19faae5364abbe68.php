...
<body>
    ...
    <table class="tablelist">
        ...
        <td><a href="<?=U('',array('id'=>$v['id']))?>" class="tablelink">修改</a>     <a href="#" class="tablelink" name="del" data-auth_id="<?php echo $v['id']?>"> 删除</a></td>
        </tr> 
        <?php }?>
	...
	$('.tablelist tbody tr:odd').addClass('odd');
        ...
        <script type="text/javascript">
            $("a[name='del']").click(function () {
                ...
                if(confirm("是否删除该规则？"))
                {
                    $.post("<?php echo U('Auth/delrule');?>",{auth_id:auth_id},function(result){
                        if(result==1)
                        {
                            alert('成功');
                            window.location.href="/authtest/index.php/Auth/ruleadd.html#tab2";
                        }
                        else
                        {
                            alert('错误');
